	...
	public static void main(String[] args) throws ParseException{
		...
		for(Person p : ps.getPeople()){
			...
			for(int i = 0;i<num_healthProfiles;i++){
				...
				int year = randBetween(Integer.parseInt(p.getBirthdate().substring(0,4)), 2013);
			    gc.set(gc.YEAR, year);
			    ...
			    hp.setDate(gc.get(gc.YEAR) + "-" + gc.get(gc.MONTH) + "-" + gc.get(gc.DAY_OF_MONTH));
			    hp.setHeartrate(randBetween(60, 90));
				hp.setMinbloodpressure(randBetween(60, 110));
				hp.setMaxbloodpressure(randBetween(hp.getMinbloodpressure() + 10, 180));
			    
			    System.out.println("[weight = " + hp.getWeight() + "] [height = " + hp.getHeight() + "] [date = " + hp.getDate() + "]");
